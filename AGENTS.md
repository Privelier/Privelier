# AGENTS.md — Project: Private Barber (working title)
...
- The `react-native-best-practices` skill (Callstack) is referenced throughout this file but is NOT actually installed (verified 2026-07-09: absent from project/global Codex skills and the harness skill list — it was never invokable). Until it is installed (tracked in the backlog), apply its intent manually: RN/Expo performance, Hermes, bundle size, native module patterns.
...
4. Use the `fullstack-developer` role through the `.codex/privelier-engineering-skiller` skill to build backend/API logic. No schema changes are allowed at this stage. For any feature involving Realtime (booking status updates, chat), also apply the `supabase-realtime-optimizer` review lens from docs/design/step-13-14-realtime-requests-retroactive-review.md plus the F1/F2 lessons; supply it the live-verified publication/replica-identity/RLS facts its checklist marks [LIVE].
...
6. Use the `ui-ux-designer` role through the `.codex/privelier-ui-ux-skiller` skill to polish the UI against the brand identity section above.
...
9. If anything breaks at any stage, use the debugging workflow in `.codex/privelier-engineering-skiller`. For Realtime-specific issues (a status or message not arriving live), check the Supabase MCP server's `get_logs` (service: realtime) and the live publication/replica-identity state first; for general data questions during debugging, query the live DB via the Supabase MCP server (`execute_sql`) instead of switching to the Supabase dashboard. (The previously-referenced `/supabase-realtime-monitor` and `/supabase-data-explorer` commands never existed — phantom references, corrected 2026-07-09.)
...
**Current Codex skill plan (2026-09-10 cleanup):** the legacy `.claude/agents`, `.claude/commands`, `.claude/scripts`, and duplicate `.claude/skills` files were removed because the skill tree was already duplicated under `.agents/skills` and the agent/command definitions were not reliably callable in this Codex runtime. Keep and use `.codex/privelier-engineering-skiller` for engineering, testing, Supabase, security, performance, and release-readiness work; use `.codex/privelier-ui-ux-skiller` for UI/UX, navigation, accessibility presentation, and visual polish. The remaining `.agents/skills` support library is intentionally small: `brainstorming` for feature/task decomposition, `supabase-postgres-best-practices` for Postgres/RLS/query review, `clean-code` for implementation hygiene, and `file-organizer` for repository cleanup. If a named agent or slash command is not actually available in the current host, apply the role's review lens manually with the retained skills and report that fallback honestly.
...
Rules:
- For codebase questions, run `npm run context:check` and then `npm run context:query -- query "<question>"` **before** opening source. The query wrapper caps graph output at 1,200 tokens. Use `path "<A>" "<B>"` or `explain "<concept>"` for relationships and focused concepts.
- If the graph is missing, run `npm run context:bootstrap`. After modifying code, run `npm run context:update`; the wrapper performs local AST extraction without an API key. Optionally run `tools/context/install-hook.sh` once for a post-commit refresh; it preserves existing hook content.
- If the graph is insufficient, use Repomix only for explicit tracked files: `npm run context:pack -- path/to/file.ts another/file.ts`. It compresses with Tree-sitter, rejects empty/broad/secret-bearing inputs, keeps its security scan enabled, enforces an 8,000-token ceiling, and writes ignored mode-0600 output.
- Read exact source ranges only after those scoped steps. Do not open whole directories, the full backlog, `GRAPH_REPORT.md`, or prior handoff/history files unless the task specifically requires a broad audit.
- These repository rules reduce code-context reads; they cannot control system prompts or chat history injected by the Codex host. Start a fresh conversation for a new feature and rely on the graph plus the relevant backlog section instead of pasting old chats.

## Active backlog (authoritative, read selectively)

The living backlog is in `docs/project/ACTIVE_BACKLOG.md`. It remains the single source of truth for what is left, but **do not load the whole file by default**. Locate the current feature with `rg -n "<feature>" docs/project/ACTIVE_BACKLOG.md`, then read only that section. Read the entire backlog only for release planning or a broad backlog audit.

Maintenance rules:
- Delete a task when its test/gate has fully passed.
- If interrupted, append `<!-- PROGRESS: what is done, what remains, where to resume -->`.
- Add newly discovered work there, not only in chat.
- Every feature still uses the mandatory pipeline above.
