...
- [ ] Stage 6 — test-engineer <!-- PROGRESS (2026-07-05, commit 29d84b4): DONE: jest-expo + testing-library infra added, unit/integration tests passing (authService error mapping + full idempotent insert-sequencing incl. 23505-as-success-of-other-writer on both users and barber_profile, errors.ts table-driven, useAuthShell phase transitions + remount-stability). tsc+eslint clean. Two Maestro flows authored (login->provisioning->role-landing, signup->await-confirmation->resend) but UNEXECUTED — no Maestro CLI installed yet. UPDATE (2026-07-06): Android dev-client tooling gap is now RESOLVED — adb/platform-tools installed and on PATH, phone authorized over USB, EAS Android development build (`eas build --profile development --platform android`) succeeded and the APK is installed on the physical device via `adb install`, Metro running via `expo start --dev-client` with `adb reverse tcp:8081 tcp:8081`. iOS dev client is still the founder's own `eas build --profile development --platform ios` (interactive Apple login, run from their own terminal, not this shell) — status unconfirmed, check `eas build:list` before re-triggering. Maestro CLI itself is STILL not installed on this machine — that is the one remaining tooling gap for running the two authored flows (on Android, at least; iOS needs its own device flow separately). REMAINS: (1) install Maestro CLI and point `maestro test` at the now-authorized Android device, (2) run the two authored flows and fix whatever breaks, (3) author + run the duplicate-email and app-kill-mid-provisioning flows (deferred on purpose until the first two run once). Item (5) from the prior note — the step-6 DB gate itself — is DONE, see below, it is no longer part of this remaining list. -->
- [x] ~~Step-6 DB gate: one fake customer + one fake barber sign up and appear in public.users (+barber_profile)~~ CONFIRMED PASSED (2026-07-06): queried auth.users + public.users directly via the Supabase Management API (read-only SQL, no service_role key involved, nothing committed to the repo). `titoacerlap@gmail.com` → public.users.role='customer', no barber_profile row (correct). `taha.metwally@outlook.de` → public.users.role='barber', barber_profile row present with verification_status='pending' (correct default). auth.users.id matches public.users.id for both. Both accounts show email_confirmed_at set and a later last_sign_in_at, so real login also works. Gate passed — delete this line's parent bullet next session once seen.
- [ ] BUG FOUND + FIXED (2026-07-06, during step-6 gate verification): email confirmation links were redirecting to `http://localhost:3000` (Supabase project's untouched default `site_url`) instead of back into the app — fails hard on any real device, no localhost:3000 exists. Root cause: `authService.signUp()`/`resend()` never passed an explicit `emailRedirectTo`, so Supabase fell back to `site_url`. IMPORTANT NUANCE explained to founders: this did NOT mean confirmation silently failed or that login worked without confirmation — Supabase's `/auth/v1/verify` endpoint sets `email_confirmed_at` server-side as part of processing the link, *before* issuing the redirect, so both existing test accounts (titoacerlap@gmail.com, taha.metwally@outlook.de) really are confirmed; the dead localhost:3000 landing page was a cosmetic dead end after the real work was already done, and both users had to manually return to the app and log in with password (no auto-session, since the tokens were stranded in that dead page's URL fragment). `mailer_autoconfirm` is `false` project-wide — signInWithPassword still correctly rejects unconfirmed accounts; there is no bypass. FIX: added `app.json` `scheme: "privelier"`, installed `expo-linking`, new `src/auth/deepLink.ts` (`getEmailRedirectTo` / `parseAuthCallbackUrl` / `applyAuthCallbackUrl`), wired `emailRedirectTo` into both `signUp()` and `resendConfirmation()`, added a `Linking` listener in `useAuthShell.ts` (cold-start via `getInitialURL` + already-running via the `url` event) that calls `setSession()` and lets the existing `onAuthStateChange` subscription do the rest — no separate state wiring. Supabase Auth config updated via the Management API: `site_url` and `uri_allow_list` both now `privelier://auth-callback` (exact match only, deliberately no wildcard, to avoid an open-redirect via a crafted `redirect_to` param). 14 new/adjusted unit tests added (`deepLink.test.ts` + updated `emailRedirectTo` assertions in `authService.test.ts`), tsc+eslint clean, full suite passing. REMAINS: the actual click-through has not been physically observed yet with the new dev client — sign up a fresh test account now that the Android dev client + Metro + adb reverse are all live, and confirm tapping the email link on the phone actually opens the app and completes the session, instead of just trusting the unit tests + config. Not yet committed/pushed — confirm with the founders before landing.
