...
- [ ] Review finding F1 (MEDIUM, blocks calling the gate passed): missed realtime events across a mid-focus reconnect are never recovered — supabase-js re-subscribes after a network blip but does not replay missed events, and the screens only refetch on focus. Fix: surface subscription-status transitions from `useBookingsRealtime` and refetch the baseline on re-`SUBSCRIBED` after an error. Fold in F2 (LOW) in the same change: the three merge call sites wrap `applyBookingChange` in `sortAsc`/`sortDesc` unconditionally, defeating its same-reference no-op contract — skip the sort when the reducer returned the same reference.
- [ ] Gate: accepting updates the booking status instantly on the customer's side via Realtime — needs a deliberate two-device (or device + second session) check with both test accounts; the single-device Maestro flow cannot cover this half and says so in its header.
