...
- [ ] Stage 7 — security gate <!-- PROGRESS (2026-07-05): DONE: static security-auditor pass (code-reviewer + security-auditor agent, /security-audit skill) completed — secrets hygiene, migration-0005 role/verification-escalation fixes, user_metadata-never-trusted-for-authz, and injection surface all confirmed clean. Result: FAIL — one HIGH finding (unauthenticated PII exposure: anon could read full users rows incl. email/phone for any approved barber via users_select_own_or_admin_or_approved_barber + the anon table grant) + two MEDIUM (anon browsing was an undecided product question; blanket `alter default privileges` grant fails open for future tables). Founder decision made: require authentication for barber discovery, never expose email/phone, discovery via a public projection only, contact-reveal deferred to the booking-flow pipeline run. Routed to supabase-schema-architect, which authored + I independently reviewed supabase/migrations/0006_require_authenticated_discovery_and_lock_users_columns.sql (revokes all anon table/schema/function access, drops the blanket default-privilege grant, replaces the leaking users policy with owner-or-admin-only, adds public.barber_directory view exposing only id/name/city/country/profile_image/bio/rating, scopes every discovery policy to authenticated). Migration is committed to the repo but NOT YET APPLIED LIVE. REMAINS: (1) restart Claude Code so the rotated SUPABASE_ACCESS_TOKEN env var takes effect for the MCP server (get_project_url works without auth and gave a false-positive signal once — list_migrations/get_advisors are the real auth check, both still return Unauthorized as of this note), (2) once live, run /supabase-schema-sync (or mcp__supabase__apply_migration) to push migration 0006, (3) re-run live negative-tests for the 0005 fixes (role flip reverted, pre-verified barber INSERT sanitized) plus new negative-tests for 0006 (anon fully locked out, barber_directory never returns email/phone), (4) run get_advisors(security) and /supabase-security-audit for the explicit PASS, (5) resolve build order step 9-10's future discovery-screen code to query public.barber_directory, never public.users, per the migration's own note. -->
